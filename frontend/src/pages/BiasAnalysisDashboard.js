    ...
    <AdminLayout>
      <div className="bias-dashboard">
        ...
        <div className="bias-tabs">
          <button className={`tab-btn ${activeTab === 'single' ? 'active' : ''}`} onClick={() => setActiveTab('single')}>Single Decision</button>
          <button className={`tab-btn ${activeTab === 'dataset' ? 'active' : ''}`} onClick={() => setActiveTab('dataset')}>Dataset Audit</button>
        </div>
        {activeTab === 'single' && <div className="analysis-section"><h2>Analyze Single Decision for Bias</h2><SingleDecisionForm onSubmit={analyzeSingleDecision} /></div>}
        {activeTab === 'dataset' && <div className="analysis-section"><h2>Audit Dataset for Fairness Issues</h2><DatasetAuditForm onSubmit={auditDataset} /></div>}
        {loading && <div className="loading">Analyzing...</div>}
        {error && <div className="error">Error: {error}</div>}
        {biasAnalysis && activeTab === 'single' && <BiasScoreDisplay analysis={biasAnalysis} />}
        {biasAnalysis && activeTab === 'dataset' && <DatasetAuditDisplay analysis={biasAnalysis} />}
      </div>
    </AdminLayout>
  );
};
...
const SingleDecisionForm = ({ onSubmit }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="decision-form">
      ...
      <div className="form-group"><label>Gender</label><select name="gender" value={formData.gender} onChange={handleChange}><option value="">Select...</option><option value="male">Male</option><option value="female">Female</option><option value="other">Other</option></select></div>
      <div className="form-group"><label>Race/Ethnicity</label><input type="text" name="race" value={formData.race} onChange={handleChange} /></div>
      <div className="form-group"><label>Credit Score</label><input type="number" name="credit_score" value={formData.credit_score} onChange={handleChange} /></div>
      <div className="form-group"><label>Decision</label><select name="decision" value={formData.decision} onChange={handleChange}><option value="approved">Approved</option><option value="denied">Denied</option><option value="pending">Pending</option></select></div>
...
const DatasetAuditForm = ({ onSubmit }) => {
  ...
  const handleFileChange = (e) => setDatasetFile(e.target.files[0]);
  const handleSubmit = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const decisions = lines.slice(1).filter(line => line.trim()).map(line => {
  ...
  return (
    <form onSubmit={handleSubmit} className="dataset-form">
      <div className="form-group"><label>Upload CSV File</label><input type="file" accept=".csv" onChange={handleFileChange} /><small>Expected columns: age, gender, race, decision, ...</small></div>
...
const BiasScoreDisplay = ({ analysis }) => {
  ...
  const getRiskColor = (level) => { switch (level) { case 'LOW': return '#28a745'; case 'MEDIUM': return '#ffc107'; case 'HIGH': return '#fd7e14'; case 'CRITICAL': return '#dc3545'; default: return '#6c757d'; } };
  return (
      ...
      <div className="fairness-table">
        ...
        {Object.entries(fairness_metrics).map(([attr, data]) => (
          <div key={attr} className="attribute-section">
            ...
            <table><thead><tr><th>Group</th><th>Total</th><th>Approved</th><th>Approval Rate</th></tr></thead><tbody>
              {Object.entries(data.stats || {}).map(([group, stats]) => (
                <tr key={group}><td>{group}</td><td>{stats.total}</td><td>{stats.approved}</td><td>{(stats.approval_rate * 100).toFixed(1)}%</td></tr>
              ))}
            </tbody></table>
            {data.disparity_detected && <div className="disparity-warning">⚠️ Significant disparity detected: {(data.disparity_gap * 100).toFixed(1)}% gap</div>}
          </div>
        ))}
      </div>
      {gemini_audit_report && <div className="audit-report"><h4>🤖 Gemini AI Audit Report:</h4><pre>{gemini_audit_report}</pre></div>}
    </div>
  );
};
